 ...
 * @param {function} [params.onAskUser] - 交互式询问用户的回调 (question) => Promise<string>
 * @returns {Promise<object>} PK 结果
 */
async function runPK(params) {
  const { prompt, contestants, judgeId, regimeId = 'ming', onAskUser } = params;
  ...
  if (judgeId) {
    ...
    // 天子御裁 — 交互式评判
    const successContestants = contestants.filter(c => results[c]?.success);
    if (successContestants.length >= 2) {
      console.log(chalk.white('  请天子裁定胜负：\n'));
      successContestants.forEach((c, i) => {
        const name = results[c]?.name || c;
        console.log(chalk.cyan(`    ${i + 1}) ${name} (${c})`));
      });
      console.log(chalk.gray(`    0) 平局 / 不分胜负`));
      console.log();

      const answer = await onAskUser(chalk.yellow('  天子裁决 (输入序号): '));
      const pick = parseInt(answer);

      if (pick >= 1 && pick <= successContestants.length) {
        const winnerId = successContestants[pick - 1];
        const winnerName = results[winnerId]?.name || winnerId;
        const loserId = successContestants.find(c => c !== winnerId);
        console.log();
        console.log(chalk.bold.yellow(`  🏆 天子钦点 ${winnerName} 获胜！`));
        reputationManager.reward(winnerId, 'pk_win');
        if (loserId) reputationManager.penalize(loserId, 'task_fail');
        console.log(chalk.gray(`  ${winnerId} +20 功勋 | ${loserId || '—'} -5 功勋`));
      } else {
        console.log(chalk.gray('\n  天子裁定：不分胜负，各赏五功勋。'));
        for (const c of successContestants) {
          reputationManager.reward(c, 'task_complete');
        }
      }
    } else {
      console.log(chalk.gray('  参赛者不足，无法评判。'));
    }
  } else {
    // 无交互 — 提示手动指定 judge
